#!/usr/bin/env bash
# Installs Mixbus from a locally downloaded upstream tarball.
set -euo pipefail

if [[ $# -ne 1 ]]; then
    echo "Usage: $0 <mixbus.tar>" >&2
    exit 1
fi

local_archive="$1"
if [[ ! -f "${local_archive}" ]]; then
    echo "Local Mixbus archive not found: ${local_archive}" >&2
    exit 1
fi
...
echo "Installing Mixbus from ${local_archive}..."
