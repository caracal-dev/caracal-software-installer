#!/usr/bin/env bash
# Installs REAPER from a locally downloaded upstream tarball.
set -euo pipefail

if [[ $# -ne 1 ]]; then
    echo "Usage: $0 <reaper-linux.tar.xz>" >&2
    exit 1
fi

local_archive="$1"
if [[ ! -f "${local_archive}" ]]; then
    echo "Local REAPER archive not found: ${local_archive}" >&2
    exit 1
fi

workdir="$(mktemp -d)"
REAPER_ARCHIVE="${workdir}/$(basename "${local_archive}")"
REAPER_EXTRACT_ROOT="${workdir}/extract"
...
echo "Installing REAPER from ${local_archive}..."
cp -f "${local_archive}" "${REAPER_ARCHIVE}"
mkdir -p "${REAPER_EXTRACT_ROOT}"
tar -xJf "${REAPER_ARCHIVE}" -C "${REAPER_EXTRACT_ROOT}"

REAPER_EXTRACT_DIR="$(find "${REAPER_EXTRACT_ROOT}" -maxdepth 2 -type f -name install-reaper.sh -printf '%h\n' | head -n 1 || true)"
if [[ -z "${REAPER_EXTRACT_DIR}" ]]; then
    echo "REAPER install-reaper.sh was not found in the archive." >&2
    exit 1
fi
