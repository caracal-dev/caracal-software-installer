...
func TestMatchRecordMatchesExactArchiveBasename(t *testing.T) {
	records := []Record{
	...
	record, err := MatchRecord(records, "/tmp/reaper765_linux_x86_64.tar.xz")
	if err != nil {
		t.Fatalf("MatchRecord returned error: %v", err)
	}
...
func TestMatchRecordMatchesPrimaryBundleName(t *testing.T) {
	records := []Record{
	...
	record, err := MatchRecord(records, "/tmp/TAL-J8X_64_linux.zip")
	if err != nil {
		t.Fatalf("MatchRecord returned error: %v", err)
	}
...
func TestMatchRecordReportsAmbiguousMatches(t *testing.T) {
	records := []Record{
	...
	_, err := MatchRecord(records, "/tmp/Floe-Linux.tar.gz")
	if err == nil || !strings.Contains(err.Error(), "Pass the package id explicitly") {
		t.Fatalf("expected ambiguous match error, got %v", err)
	}
}

func testRecord(id string, name string, entry downloadindex.Entry) Record {
	if entry["name"] == "" {
		entry["name"] = name
	}
	return Record{
